class VendorsController < AuthenticatedController
	...
	def assign_vendor_to_collection
		render json: { success: false } and return if params[:collection_id].blank?
		collection = ShopifyAPI::CustomCollection.find(params[:collection_id])
		Vendor.find_by(id: params[:id])&.update(collection_id: params[:collection_id], collection_title: collection.handle)
		render json: { success: true, message: 'successfully assigned collection to vendor' }
	end
